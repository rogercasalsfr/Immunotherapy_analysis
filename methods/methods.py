...
def _resolve_start_node(adata, celltype_key, start_node):
    if isinstance(start_node, str):
        categories = np.unique(adata.obs[celltype_key].astype(str).values)
        matches = np.where(categories == start_node)[0]
        if matches.size == 0:
            raise ValueError(
                f"start_node celltype '{start_node}' was not found in adata.obs['{celltype_key}']. "
                f"Available categories: {categories.tolist()}"
            )
        return int(matches[0])

    num_clusters = adata.obs[celltype_key].astype(str).nunique()
    if start_node >= num_clusters:
        raise ValueError(
            f"start_node={start_node} is out of range for {num_clusters} clusters in "
            f"adata.obs['{celltype_key}']. Pass the celltype name instead to avoid mismatches."
        )
    return start_node
...
def run_slingshot(
    ...
    resolved_start_node = _resolve_start_node(adata, "celltype", start_node)

    slingshot = Slingshot(
        adata,
        celltype_key="celltype",
        obsm_key="X_umap",
        start_node=resolved_start_node,
